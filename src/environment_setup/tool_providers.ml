...
let cibersort_tool
    ~(run_program : Machine.Make_fun.t)
    ~host ~install_tools_path loc =
  let cibersort = Machine.Tool.Default.cibersort in
  let open KEDSL in
  let install_path =
    install_tools_path // (Tool_def.to_directory_name cibersort)
  in
  let required_setup_pkgs =
    [ ("java-jdk", `Version "7");
      (* Yup! It's a Java app that needs
         an Rserve to be able to do the statistic.
         So no mistakes here, it is just the way it is.
         Here we are installing all the R dependencies
         through conda since it is a more reliable
         environment that we know. *)
      ("r", `Version "3");
      (* The R package's versioning is a bit weird on conda
         but it does a great job matching it with the base R;
         so we will let it deal the versions: *)
      ("r-rserve", `Latest);
      ("r-e1071", `Latest);

      ("bioconductor-preprocesscore", `Latest); ]
      (* In addition to the above dependencies,
         the README also lists the R packages
         `parallel` and `colorRamps`. The former is now
         officialy part of the R so we will ignore it.
         The latter is not available on Conda, we have to
         do it manually. *)
  in
  let conda_env =
    Conda.(setup_environment
  ...
  let conda_init = Conda.init_env ~conda_env () in
  let get_cibersort =
    Workflow_utilities.Download.get_tool_file ~identifier:"cibersort"
  ...
  let make =
    run_program
      ~requirements:[
        `Internet_access;
        `Self_identification ["customized-installation"; "CIBERSORT"];
      ]
      Program.(
        shf "cd %s" install_path
        && sh "unzip CIBERSORT_package.zip"
        && conda_init
        && sh "Rscript -e \"source('http://bioconductor.org/biocLite.R'); \
               biocLite(c('colorRamps'));\""
      )
  in
  let ciber_install = workflow_node products ~name ~edges ~make in
  let init =
